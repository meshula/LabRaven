...
{
    //lc_m44f m = transpose(rotation_matrix_from_quat(rt->orientation)); // for debugging
    lc_v3f eye = rt->position;
    lc_v3f x = mul(qxdir(rt->orientation), rt->scale.x);
    lc_v3f y = mul(qydir(rt->orientation), rt->scale.y);
    lc_v3f z = mul(qzdir(rt->orientation), rt->scale.z);
    return lc_m44f{ lc_v4f{ x.x, y.x, z.x, 0.f },
                    lc_v4f{ x.y, y.y, z.y, 0.f },
                    lc_v4f{ x.z, y.z, z.z, 0.f },
                    lc_v4f{ -dot(x, eye), -dot(y, eye), -dot(z, eye), 1.f } };
}
    ...
    lc_i_Mode mode, lc_v2f delta_in, lc_radians roll_hint, float /*dt*/)
{
    ...
    case lc_i_ModeTurnTableOrbit:
        i->_turntable(*camera, { delta.x, delta.y });
        lc_i_set_roll(i,camera, tok, roll_hint);
        break;
                          ...
                          lc_v2f current_mouse_, lc_radians roll_hint, float dt)
{
    ...
    case lc_i_ModePanTilt:
    {
        ...
        {
            i->_init_mouse = current_mouse_;
            i->_initial_inv_projection = lc_camera_inv_view_projection(camera,
                                                                       i->_viewport_size.x / i->_viewport_size.y);
...
extern "C"
void lc_i_set_fixed_orbit_center(lc_interaction* i, bool f) {
    i->_orbit_fixed = f;
}
...
extern "C"
lc_v3f lc_mount_forward(const lc_mount* mnt)
{
    return qzdir(mnt->transform.orientation);
}
...
{
    const lc_rigid_transform* cmt = &cam->mount.transform;
    lc_m44f inv_projection = lc_camera_inv_view_projection(cam, viewport_size.x / viewport_size.y);
