...
int nanoexr_read_header(nanoexr_Reader_t* reader, exr_read_func_ptr_t readFn,
    ...
    exr_result_t chroma_rv = exr_attr_get_chromaticities(exr, partIndex,
                                                        "chromaticities",
                                                        &chroma);

    if (chroma_rv == EXR_ERR_SUCCESS) {
        NcChromaticity redPrimary = { chroma.red_x, chroma.red_y };
        NcChromaticity greenPrimary = { chroma.green_x, chroma.green_y };
        NcChromaticity bluePrimary = { chroma.blue_x, chroma.blue_y };
        NcChromaticity whitePoint = { chroma.white_x, chroma.white_y };

        const char* cs = NcMatchLinearColorSpace(redPrimary, greenPrimary, bluePrimary,
                                                 whitePoint, 1.0e-3f);
        if (cs) {
            printf("matched color space: %s\n", cs);
        }
        else {
            printf("Unable to match color space\n");
        }
    }
    else {
        printf("Chromaticity data not in EXR file\n");
    }
                                    ...
                                    int* rgbaIndex)
{
    ...
    do {
        ...
        int tileYStride = tilew * pixelStride;
