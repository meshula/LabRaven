...
//-------------------------------------------------------------------------
void Demo_ColorFilters(const std::string& renderCS);
void Demo_ColorMatrix(const std::string& renderCS);
void Demo_TextureAnnotations(const std::string& renderCS);
void Demo_AlphaMode(const std::string& renderCS);
void Demo_WrapAndFilter(const std::string& renderCS);
void ShowDemoWindow(const std::string& renderCS);
        ...
        if (colorspaces[n] != nullptr) {
            do {
                ...
                if (ImGui::Selectable(colorspaces[n], is_selected)) {
                    ...
                    auto d2i = NcGetRGBToRGBTransform(&ics, &dcs);
                    auto& m = d2i.transform.m;
                    float matrix[16] = { m[0], m[1], m[2], 0,
                                         m[3], m[4], m[5], 0,
                                         m[6], m[7], m[8], 0,
                                         0, 0, 0, 1};
        ...
        if (colorspaces[n] != nullptr) {
            do {
                ...
                if (ImGui::Selectable(colorspaces[n], is_selected)) {
                    ...
                    float colorOffset[] = {0,0,0, 0};
                    ImGuiTexInspect::CurrentInspector_SetColorMatrix(matrix, colorOffset);
                    ImGuiTexInspect::CurrentInspector_SetGamma(dcs.gamma);
                }
                if (is_selected)
                    ImGui::SetItemDefaultFocus();
                ++n;
...
void ShowDemoWindow(const std::string& renderingCS)
{
    ...
    struct DemoConfig
    {
        ...
        void (*DrawDemoFn)(const std::string&);    // Function which implements the demo
    };
    ...
    if (ImGui::Begin("Texture Inspector"))
    {
        ...
        demos[selectedDemo].DrawDemoFn(renderingCS);
    }
    ImGui::End();
}
