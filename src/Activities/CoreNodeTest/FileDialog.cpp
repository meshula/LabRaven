...
bool FileDialog::Draw(bool* open)
{
    ...
    if (ImGui::BeginPopupModal(title.c_str(), open, ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoDocking))
    {
        ...
        int w = ImGui::GetWindowContentRegionMax().x - ImGui::GetWindowContentRegionMin().x;
        ImGui::BeginChild("##browser", ImVec2(w, 300.0f), true, ImGuiWindowFlags_None);
        size_t index = 0;
        ...
        {
            int w = ImGui::GetWindowContentRegionMax().x - ImGui::GetWindowContentRegionMin().x;
            if (ImGui::Selectable("..", currentIndex == index, ImGuiSelectableFlags_AllowDoubleClick, ImVec2(w, 0)))
            {
                currentIndex = index;
        ...
        for (const auto& element : currentDirectories)
        {
            ...
            int w = ImGui::GetWindowContentRegionMax().x - ImGui::GetWindowContentRegionMin().x;
            if (ImGui::Selectable(element.path().filename().string().c_str(), currentIndex == index, ImGuiSelectableFlags_AllowDoubleClick, ImVec2(w, 0)))
            {
                currentIndex = index;
                if (ImGui::IsMouseDoubleClicked(0))
                {
