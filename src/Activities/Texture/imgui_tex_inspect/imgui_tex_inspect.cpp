...
void CurrentInspector_SetColorMatrix(const float (&matrix)[16])
{
    Inspector *inspector = GContext->CurrentInspector;
...
void CurrentInspector_SetInputGamma(float gamma)
{
    Inspector *inspector = GContext->CurrentInspector;
    ShaderOptions *shaderOptions = &inspector->ActiveShaderOptions;
    shaderOptions->InputColorspace.gamma = gamma;
    shaderOptions->InputColorspace.linearBias = 0;
}
...
void CurrentInspector_SetOutputGamma(float gamma)
{
    Inspector *inspector = GContext->CurrentInspector;
    ShaderOptions *shaderOptions = &inspector->ActiveShaderOptions;
    shaderOptions->OutputColorspace.gamma = gamma;
    shaderOptions->OutputColorspace.linearBias = 0;
}
...
{
    if (!GContext || !GContext->CurrentInspector)
        return;
    ...
    ImGui::InputFloat("###tdi0", &shaderOptions->InputColorspace.gamma);
    ImGui::PopItemWidth();
    ImGui::SameLine();
    ImGui::TextUnformatted("Exposure:");
    ImGui::SameLine();
    ImGui::PushItemWidth(40);
    ImGui::InputFloat("###tdi1", &shaderOptions->exposure);
    ImGui::PopItemWidth();
    ImGui::SameLine();
    ImGui::TextUnformatted("Lift:");
    ImGui::SameLine();
    ImGui::PushItemWidth(40);
    ImGui::InputFloat("###tdi2", &shaderOptions->lift);
    ImGui::PopItemWidth();
    ...
    ImGui::SameLine();
    ImGui::PushItemWidth(40);
    ImGui::InputFloat("###ti0", &shaderOptions->OutputColorspace.gamma);
    ImGui::PopItemWidth();
    ImGui::SameLine();
    ImGui::TextUnformatted("Exposure:");
    ImGui::SameLine();
    ImGui::PushItemWidth(40);
    ImGui::InputFloat("###ti1", &shaderOptions->exposure);
    ImGui::PopItemWidth();
    ImGui::SameLine();
    ImGui::TextUnformatted("Lift:");
    ImGui::SameLine();
    ImGui::PushItemWidth(40);
    ImGui::InputFloat("###ti2", &shaderOptions->lift);
    ImGui::PopItemWidth();
}
...
ImVec4 GetTexel(const BufferDesc *bd, int x, int y)
{
    ...
    else if (bd->Data_half)
    {
        const uint16_t *texel = bd->Data_half + offset;
        return ImVec4(                   half_to_float(texel[bd->Red]),
                bd->ChannelCount >= 2 ?  half_to_float(texel[bd->Green]) : 0,
                bd->ChannelCount >= 3 ?  half_to_float(texel[bd->Blue])  : 0,
                bd->ChannelCount >= 4 ?  half_to_float(texel[bd->Alpha]) : 0);
    }
...
void ValueText::DrawAnnotation(ImDrawList *drawList, ImVec2 texel, Transform2D texelsToPixels, ImVec4 value)
{
    ...
    * won't work on other fonts which may even not be monospace.*/
    ...
    if (FormatAsFloats)
    {
        snprintf(buffer, sizeof(buffer), TextFormatString, value.x, value.y, value.z, value.w);
    }
    else
    {
        ...
        snprintf(buffer, sizeof(buffer), TextFormatString, r, g, b, a);
    }
